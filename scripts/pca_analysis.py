...
def main():
    ...
    # Remove extreme outliers (Z-score > 3) to improve PCA visual integrity
    from scipy import stats
    z_scores = np.abs(stats.zscore(df_model[required_cols]))
    # Keep only rows where all required columns have z-score < 3
    df_model = df_model[(z_scores < 3).all(axis=1)]
    
    final_len = len(df_model)
    print(f"Removed {initial_len - final_len} rows (missing values + extreme outliers).")
